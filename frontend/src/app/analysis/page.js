    ...
    const singleIteration = async () => {
      try {
        ...
        if (gdRes.ok) {
          ...
          // Sort entries by ID in descending order to get newest first
          const sortedEntries = responseEntries.sort((a, b) => b.id - a.id);

          setCollectedEntries((prevEntries) => {
            // Get the highest ID we've seen before
            const highestPrevId = prevEntries.length > 0 
              ? Math.max(...prevEntries.map(e => e.id))
              : -1;

            // Filter for entries with higher IDs than what we've seen
            const newEntries = sortedEntries.filter(entry => entry.id > highestPrevId);

            if (newEntries.length > 0) {
              // Add new entries to the beginning of the array and keep only last 2 entries
  ...
  const startModelTraining = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setTrainMessage("Model training completed successfully! You can now analyze your sample in the dashboard.");
      } else {
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <section className="bg-white rounded-xl shadow p-6">
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-yellow-700">
                    Readings complete, no sample in enclosure. You can now proceed with model training.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-gray-500 mb-2">Auto-updating with newest data every 15 seconds</p>
            <div className="bg-gray-50 p-3 rounded">
              {collectedEntries.length ? (
                <div className="space-y-4">
                  {collectedEntries.slice(0, 2).map((entry, index) => (
                    <div key={entry.id} className={`${index === 0 ? 'border-l-4 border-green-500' : 'border-l-4 border-gray-300'} pl-3 pb-2 ${index === 0 ? 'bg-green-50' : ''}`}>
                      <div className="flex justify-between text-sm">
                        <span className="font-medium">
                          {index === 0 ? 'Newest Entry' : 'Previous Entry'} (ID: {entry.id})
                        </span>
                ...
                <p className="text-sm text-gray-500">No new data yet. Waiting for entries...</p>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-xl shadow p-6">
          ...
          <div className="flex items-center gap-3 mb-4">
            <button
              onClick={async () => {
                try {
                  const res = await fetch(`${BASE_URL}/getdata/${factoryMedicineId}`);
                  if (!res.ok) {
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Salty</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bitter</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sour</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Umami</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Quality</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dilution</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {collectedEntries.map((entry) => (
                    <tr key={entry.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.id}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {new Date(entry.timestamp).toLocaleString()}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.taste_sweet}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.taste_salty}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.taste_bitter}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.taste_sour}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.taste_umami}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.quality}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">{entry.dilution}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
